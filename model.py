...
class PixelNorm(nn.Module):
    ...
    def forward(self, x):
        return x / flow.sqrt(flow.mean(x ** 2, dim=1, keepdim=True) + self.epsilon)
...
class Generator(nn.Module):
    ...
    def fade_in(self, alpha, upscaled, generated):
        ...
        return flow.tanh(alpha * generated + (1 - alpha) * upscaled)
...
class Discriminator(nn.Module):
    ...
    def minibatch_std(self, x):
        ...
        # ori
        batch_statistics = (flow.std(x, dim=0).mean().repeat(x.shape[0], 1, x.shape[2], x.shape[3]))

        # to test: oneflow cannot repeat tensor(nan)
        # test = flow.Tensor(np.array([0]).astype(np.int32))
        # batch_statistics = test.repeat(x.shape[0], 1, x.shape[2], x.shape[3])

        # we take the std for each example (across all channels, and pixels) then we repeat it
        # for a single channel and concatenate it with the image. In this way the discriminator
        # will get information about the variation in the batch/image
        return flow.cat([x, batch_statistics], dim=1)
